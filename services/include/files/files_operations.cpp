...
NAsync::TAsyncTask<bool> WriteAllAsync(NAsync::TReactorPtr reactor, int fd, std::span<const char> buffer, uint64_t offset) {
    size_t sent = 0;
...
NAsync::TAsyncTask<bool> ReadExactAsync(NAsync::TReactorPtr reactor, int fd, std::span<char> buffer, uint64_t offset) {
    size_t received = 0;
